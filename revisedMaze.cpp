...
/**
* Get the currently constructed maze and write to an outfile
* 
**/
void output_maze(){
    ...
    // This cannot print out a maze yet because of some bug
    for(int i = 0; i < MAZE_SIZE; i++){
        for(int j = 0; j < MAZE_SIZE; j++){
...
void init_dist() {
    ...
    for (int i = 0; i < MAZE_SIZE; i++) {
        for (int j = 0; j < MAZE_SIZE; j++) {
            // TESTING 
            // cout<< " ( " << i << " , " << j << " ):  " << min4(manhattan_dist(i, goal1, j, goal1),
            //    manhattan_dist(i, goal1, j, goal2),
            //    manhattan_dist(i, goal2, j, goal1),
            //    manhattan_dist(i, goal2, j, goal2)) << endl;
            // Distance of the cell will be the minimum distance to the closest
            // one out of four middle destination cells.
